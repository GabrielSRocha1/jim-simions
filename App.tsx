    ...
    <div className={`max-w-7xl mx-auto px-4 py-8 md:py-12 space-y-10 transition-colors duration-1000 ${isAggressive ? 'aggressive-mode' : ''}`}>
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-gray-400 font-bold text-sm ml-1 md:ml-15 uppercase tracking-widest opacity-80">
            {t.header_subtitle}
          </p>
        </div>
        
        <div className="flex flex-col md:items-end gap-4">
          <div className="flex gap-3 glass p-1.5 rounded-xl border-white/5 shadow-2xl">
            {(['pt', 'en', 'es'] as Language[]).map(l => (
              <button 
                key={l}
                onClick={() => setLang(l)}
                className={`w-12 h-8 rounded-lg transition-all overflow-hidden border ${lang === l ? 'border-cyan-400 shadow-[0_0_10px_rgba(0,242,255,0.3)]' : 'border-transparent opacity-30 grayscale hover:opacity-100'}`}
              >
          ...
          <button 
            ...
            className="group relative px-8 py-3 bg-white/5 hover:bg-white/10 rounded-2xl border border-white/10 transition-all active:scale-95 disabled:opacity-50 overflow-hidden shadow-xl"
          >
            ...
            <div className="relative flex items-center gap-3">
              ...
              <span className="font-black tracking-widest uppercase text-[10px]">{t.cta_analyze}</span>
            </div>
          </button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <aside className="lg:col-span-4 space-y-6">
          <div className={`glass p-8 rounded-[2.5rem] space-y-8 shadow-2xl border transition-all duration-700 ${isAggressive ? 'border-yellow-500/30' : 'border-white/5'}`}>
            ...
            <div className="space-y-6">
              <div className="p-4 rounded-2xl bg-white/5 border border-white/5 group hover:border-cyan-400/30 transition-all">
                ...
                />
                <div className="text-[13px] font-black font-mono text-white/70 tracking-widest mt-2 bg-white/5 p-2 rounded-lg border border-white/5 shadow-inner">
              ...
              <div className="p-6 rounded-3xl bg-white/10 border border-cyan-400/40 group hover:border-cyan-400/60 transition-all shadow-2xl shadow-cyan-400/10 scale-[1.02]">
                <div className="flex justify-between items-center mb-3">
                  <label className="text-[11px] font-black text-cyan-400 uppercase tracking-[0.2em]">{t.total_investment}</label>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <input 
                      ...
                      className="bg-transparent font-mono text-3xl font-black text-cyan-400 focus:outline-none w-full border-b-2 border-cyan-400/20 pb-1"
                    />
                    <div className="text-[18px] font-black font-mono text-cyan-400 tracking-tighter py-2 px-3 bg-cyan-900/30 rounded-xl border border-cyan-400/20 flex justify-between">
                      <span>$ {formatNumber(state.investmentUsd)}</span>
                      <span className="text-[10px] text-cyan-500/50 uppercase">Allocated Cap</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <input 
                      ...
                      value={state.investmentUsd}
                      onChange={(e) => handleInvestmentChange(Number(e.target.value))}
                      className="w-full h-2.5 bg-cyan-900/50 rounded-lg appearance-none cursor-pointer accent-cyan-400"
                    />
                    <div className="flex justify-between text-[11px] font-black text-gray-500 uppercase tracking-tighter px-1">
                      <span>$1k</span>
                      <span>$10M</span>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{t.lock_duration}</label>
              ...
              <div className="space-y-5 p-6 rounded-3xl bg-black/20 border border-white/5">
                ...
                <div className="space-y-3">
                  ...
                  <input type="range" min="0" max="2000" step="50" value={state.newRoudersPerMonth} onChange={(e) => handleCommunityGrowthChange(Number(e.target.value))} className="w-full accent-orange-500" />
                </div>
                <div className="flex justify-between items-center bg-black/40 p-3 rounded-xl border border-white/5">
                  <span className="text-[9px] font-bold text-gray-400 uppercase">{t.avg_ticket}</span>
                  <input type="number" value={state.ticketPerRouder} onChange={(e) => setState(s => ({...s, ticketPerRouder: Number(e.target.value)}))} className="bg-transparent text-right font-mono text-xs font-bold text-white w-20 focus:outline-none" />
                </div>
              </div>
            ...
            <button className="w-full cta-glow py-6 rounded-[2rem] text-xs uppercase tracking-[0.4em] shadow-2xl pulse-glow group relative overflow-hidden transition-all active:scale-95">
        ...
        <main className="lg:col-span-8 space-y-8">
          <div className="glass p-8 rounded-[3rem] relative overflow-hidden shadow-2xl border border-white/5">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 relative z-10">
              <div className="space-y-1">
                <h3 className="text-2xl font-black tracking-tighter uppercase italic">{t.growth_chart_title}</h3>
                <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest opacity-60">{t.growth_chart_sub}</p>
              </div>
              <div className="flex flex-wrap gap-4 bg-black/20 p-3 rounded-2xl border border-white/5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="glass p-10 rounded-[2.5rem] shadow-lg border border-white/5 space-y-8">
               ...
               <div className="space-y-6">
                 ...
                 <AllocationBar label={t.price_gain} value={results.appreciationGain} total={results.totalFutureValue} color="bg-purple-500 shadow-[0_0_15px_rgba(188,0,255,0.4)]" />
                 <AllocationBar label={t.yield_gain} value={results.yieldGain} total={results.totalFutureValue} color="bg-cyan-500 shadow-[0_0_20px_rgba(0,242,255,0.4)]" />
               </div>
            </div>
            ...
            <div className={`glass p-10 rounded-[2.5rem] min-h-[220px] transition-all duration-1000 shadow-xl border ${insights ? 'border-cyan-500/30' : 'border-white/5 opacity-40'}`}>
              ...
              {insights ? (
                <div className="text-xs text-gray-200 leading-relaxed font-bold italic overflow-auto max-h-[140px] scrollbar-thin">
                  "{insights}"
                </div>
              ) : (
...
const MetricCard = ({ label, value, type, glow, sub }: any) => {
  ...
  return (
    <div className={`glass p-6 md:p-8 rounded-[2.5rem] border-b-4 transition-all hover:-translate-y-1 shadow-2xl flex flex-col justify-between ${colors[glow]}`}>
      ...
      <p className="text-[9px] text-gray-500 font-black mt-4 uppercase tracking-tighter opacity-70 border-t border-white/5 pt-2">{sub}</p>
    </div>
  );
};
...
const TierSelector = ({ active, onClick, label, duration, yieldPct, color }: any) => (
  <button onClick={onClick} className={`p-4 rounded-2xl border transition-all flex items-center justify-between group overflow-hidden relative ${active ? 'bg-white/10 border-white/30 shadow-2xl scale-[1.03] z-10' : 'bg-transparent border-white/5 opacity-40 hover:opacity-100 hover:scale-[1.01]'}`}>
    ...
    <div className="relative z-10 text-right">
      <div className="text-xl font-mono font-black text-gradient-cyan">{yieldPct}</div>
    </div>
...
const LegendItem = ({ color, label, dashed }: any) => (
  <div className="flex items-center gap-2">
    <div className="w-3 h-0.5" style={dashed ? {borderTop: `2px dashed ${color}`, background: 'transparent'} : {backgroundColor: color}} />
...
const ChartTooltip = ({ active, payload, label, t, lang }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="glass p-6 rounded-[2.5rem] border-white/20 shadow-2xl space-y-5 min-w-[280px]">
        <div className="flex justify-between items-center border-b border-white/5 pb-3">
          <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{t.week} {label}</span>
          <span className="text-[10px] font-black text-cyan-400 px-3 py-1 rounded bg-cyan-400/10 uppercase italic">{t.total_balance}</span>
        </div>
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center text-[10px] bg-green-500/10 p-2 rounded-xl border border-green-500/20">
          ...
          <div className="flex justify-between items-center border-t border-white/5 pt-3 text-[10px]">
        ...
        <p className="text-[9px] text-gray-500 italic text-center opacity-60 uppercase font-black tracking-widest">{t.tooltip_month_prefix}</p>
        <p className="text-xl font-mono font-black text-white text-center">{format(data.totalValue)}</p>
      </div>
    );
  }
  return null;
